...
def characteristic_function(s, t):
    return (np.exp(np.complex(0, 1) * s) ** t.reshape(-1, 1)).mean(axis=1)


def apply_kernel(idx, N, kernel):
    f = np.zeros(N)
    f[idx] = 1
    return kernel.analyze(f)
...
# Apply kernel
%timeit heat_print = heat_kernel.analyze(np.eye(pG.N)).transpose((2, 0, 1))
...
for i, sig in enumerate(heat_print):
    sig_feats = []
    for node in sig:
        node_feats = characteristic_function(node, np.arange(0, 100, 2))
        node_feats = np.column_stack([node_feats.real, node_feats.imag])
        node_feats = node_feats.reshape(-1)
